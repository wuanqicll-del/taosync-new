<template>
	<div class="notify">
		<div class="loading-box content-none-data" v-if="loading">加载中</div>
		<div v-else class="card-box">
			<div class="card-item glass-card" v-for="item in dataList">
				...
				<div class="card-item-bottom">
					...
					<el-button size="small" type="primary" @click="tstCu(item)">测试</el-button>
					<el-button size="small" type="danger" @click="delCu(item.id)">删除</el-button>
				</div>
			</div>
			<div class="card-item card-add glass-card" @click="addShow" v-if="!loading">
				<template v-if="dataList.length == 0">
					暂无通知配置，请新增
				</template>
				<span v-else>新增</span>
			</div>
			<el-dialog top="6vh" :visible.sync="editShow" :title="editFlag ? '编辑' : '新增'"
					...
					<el-form :model="editData" :rules="editRule[editData.method]" ref="addRule" v-if="editShow"
						...
						<template v-else-if="editData.method == 5">
							<el-form-item prop="params.url" label="地址">
								<el-input v-model="editData.params.url"
									placeholder="http://你的ntfy地址:端口/你的topic"></el-input>
							</el-form-item>
						</template>
				...
				<span slot="footer" class="dialog-footer">
					...
					<el-button type="success" @click="tstCu()">测 试</el-button>
...
<style lang="scss">
	.notify {
		...
		.card-item-top {
			...
			.enable-enable { color: var(--color-success); }
			.enable-disable { color: var(--color-danger); }
		}
