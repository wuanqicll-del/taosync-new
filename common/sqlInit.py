...
def init_sql(conn):
    ...
    if cursor.fetchone() is None:
        ...
        cursor.execute("create table job("
                       ...
                       "syncRule integer DEFAULT 1,"        # 同步规则，1-数据库对比，2-大小+名称，3-大小+名称+时间
                       "perPage integer DEFAULT 200,"       # 每页获取文件数量，1-500
                       "overwriteDelete integer DEFAULT 0," # 覆盖前删除，0-关闭，1-开启
                       "deleteDelay integer DEFAULT 0,"   # 删除后等待秒数，用于异步删除的网盘（如115）
                       "createTime integer DEFAULT (strftime('%s', 'now')),"
        ...
        cursor.execute("create table file_cache("
                       "id integer primary key autoincrement,"
                       "jobId integer,"             # 所属工作id
                       "relPath text,"              # 相对于源根目录的路径
                       "fileSize integer,"          # 文件大小
                       "fileModified text,"         # 源端修改时间"
                       " unique (jobId, relPath))")
        conn.commit()
    else:
        try:
        ...
        if sqlVersion < cuVersion:
            ...
            if sqlVersion < 25060801:
                cursor.execute("alter table job_task add column finishTime integer DEFAULT NULL")
            if sqlVersion < 25060901:
                cursor.execute("create table file_cache("
                               "id integer primary key autoincrement,"
                               "jobId integer,"
                               "relPath text,"
                               ...
                               "fileModified text,"
                               " unique (jobId, relPath))")
            if sqlVersion < 25060902:
                cursor.execute("alter table job add column syncRule integer DEFAULT 1")
            if sqlVersion < 25060903:
                cursor.execute("alter table job add column perPage integer DEFAULT 200")
            if sqlVersion < 25060904:
                cursor.execute("alter table job add column overwriteDelete integer DEFAULT 0")
            if sqlVersion < 25060905:
                cursor.execute("alter table job add column srcDirName integer DEFAULT 1")
            if sqlVersion < 25060906:
                cursor.execute("alter table job add column deleteDelay integer DEFAULT 0")
