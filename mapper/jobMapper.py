...
def addJob(job):
    ...
    return sqlBase.execute_insert("insert into job (enable, remark, srcPath, dstPath, alistId, useCacheT, "
                                  ...
                                  "start_date, end_date, exclude, syncRule, perPage, overwriteDelete, srcDirName, deleteDelay) "
                                  "VALUES (:enable, :remark, :srcPath, :dstPath, :alistId, :useCacheT, "
                                  ":scanIntervalT, :useCacheS, :scanIntervalS, :method, :interval, "
                                  ":isCron, :year, :month, :day, :week, :day_of_week, :hour, :minute, :second, "
                                  ":start_date, :end_date, :exclude, :syncRule, :perPage, :overwriteDelete, :srcDirName, :deleteDelay)", job)
...
def updateJob(job):
    ...
    sqlBase.execute_update("update job set enable=:enable, remark=:remark, srcPath=:srcPath, dstPath=:dstPath, alistId=:alistId, "
                           ...
                           "second=:second, start_date=:start_date, end_date=:end_date, exclude=:exclude, syncRule=:syncRule, perPage=:perPage, overwriteDelete=:overwriteDelete, srcDirName=:srcDirName, deleteDelay=:deleteDelay where id=:id",
                           job)
...
def deleteJob(jobId):
    ...
    sqlBase.execute_update("delete from file_cache where jobId=?", (jobId,))
    sqlBase.execute_update("delete from job where id=?", (jobId,))


def clearFileCache(jobId):
    sqlBase.execute_update("delete from file_cache where jobId=?", (jobId,))
...
def updateJobTaskStatus(taskId, status, errMsg=None):
    sqlBase.execute_update(f"update job_task set status=?, errMsg=?, finishTime=CAST(strftime('%s','now') AS INTEGER) where id=?", (status, errMsg, taskId))
...
def getFileCache(jobId):
    """获取某任务的源端文件缓存，返回 {relPath: (size, modified)}"""
    rows = sqlBase.fetchall_to_table(
        "select relPath, fileSize, fileModified from file_cache where jobId=?", (jobId,))
    return {row['relPath']: (row['fileSize'], row['fileModified']) for row in rows}


def addFileCacheOne(jobId, relPath, fileSize, fileModified):
    """单条写入缓存（INSERT OR REPLACE）"""
    sqlBase.execute_update(
        "insert or replace into file_cache (jobId, relPath, fileSize, fileModified) values (?, ?, ?, ?)",
        (jobId, relPath, fileSize, fileModified))


def addFileCacheMany(jobId, fileList):
    """批量写入缓存（INSERT OR REPLACE）"""
    if not fileList:
        return
    sqlBase.execute_manny(
        "insert or replace into file_cache (jobId, relPath, fileSize, fileModified) values (?, ?, ?, ?)",
        [(jobId, relPath, fileSize, fileModified) for relPath, fileSize, fileModified in fileList])


def deleteFileCacheByDir(jobId, dirPath):
    """删除指定目录下所有文件的缓存记录"""
    sqlBase.execute_update("delete from file_cache where jobId=? and relPath like ?",
                           (jobId, dirPath + '%'))


def deleteFileCacheOne(jobId, relPath):
    """删除单条缓存记录"""
    sqlBase.execute_update("delete from file_cache where jobId=? and relPath=?",
                           (jobId, relPath))
