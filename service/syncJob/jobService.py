...
def cleanJobInput(job):
    ...
    if job['srcPath'] is not None:
        job['srcPath'] = ":".join([item.strip() for item in job['srcPath'].split(':')])
    if job['dstPath'] is not None:
        job['dstPath'] = ":".join([item.strip() for item in job['dstPath'].split(':')])
...
def clearFileCache(jobId):
    jobMapper.clearFileCache(int(jobId))
