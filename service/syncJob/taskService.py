...
def updateJobTaskStatus(taskId, status, errMsg=None, taskList=None, createTime=None):
    ...
    if notifyList:
        ...
        # 只有出现失败或异常才发通知，只发标题不发正文
        if status != 2 or taskNum['failNum'] > 0 or taskNum['otherNum'] > 0:
            remark = job['remark'] if job['remark'] else ''
            notifyTitle = f"{remark}：同步失败" if remark else "同步失败"
            for notify in notifyList:
                try:
                    notifyService.sendNotify(notify, notifyTitle, '', needNotSync)
...
def getTaskList(req):
    ...
    if needUpdateList:
        ...
        updateThread.start()
    # 对 status=1 的运行中任务，从内存取当前阶段和进度
    try:
        from service.syncJob import jobService
        jobId = req.get('id')
        if jobId:
            client = jobService.getJobClientById(int(jobId))
            if client and client.currentJobTask:
                ct = client.currentJobTask
                for item in jobTaskList['dataList']:
                    if item['status'] == 1:
                        phase = ct.currentPhase
                        item['phase'] = phase
                        if phase == 'scanning':
                            item['totalFiles'] = len(ct.scannedSrcFiles)
                            item['completedFiles'] = 0
                            item['pendingFiles'] = len(ct.pendingMkdirs) + len(ct.pendingCopies) + len(ct.pendingDeletes)
                        elif phase == 'mkdir':
                            item['totalFiles'] = ct.mkdirTotal
                            item['completedFiles'] = ct.mkdirDone
                            item['pendingFiles'] = 0
                        elif phase == 'pre-delete':
                            item['totalFiles'] = ct.preDeleteTotal
                            item['completedFiles'] = ct.preDeleteDone
                            item['pendingFiles'] = 0
                        else:
                            # executing
                            item['totalFiles'] = ct.pendingFileCount
                            item['completedFiles'] = len(ct.finish)
                            item['pendingFiles'] = 0
    except Exception:
        pass
    return jobTaskList
