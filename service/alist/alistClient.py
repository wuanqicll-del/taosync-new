...
def checkExs(path, rts, spec):
    ...
    for rtsItem in rts.keys():
        checkPath = (path + '/' + rtsItem).replace('//', '/')
        if spec.match_file(checkPath):
            del rtsNew[rtsItem]
    return rtsNew
...
class AlistClient:
    ...
    def fileListApi(self, path, useCache=0, scanInterval=0, spec=None, rootPath=None, perPage=200):
        """
        目录列表（自动分页获取全部文件）
        ...
        :return: {
            "test1-1/": {},
            "test1.txt": (4, "2024-01-01T00:00:00Z")
        }
        """
        ...
        while True:
            ...
            if content is None:
                if firstPage:
                    # 第一页content为空，可能是存储驱动报错，尝试获取完整响应检查message
                    try:
                        headers = {'Authorization': self.token} if self.token else None
                        r = requests.post(self.url + '/api/fs/list', json={
                            'path': path,
                            'refresh': useCache != 1,
                            'page': page,
                            'per_page': perPage
                        ...
                        fullRes = r.json()
                        msg = fullRes.get('message', '')
                        if msg and 'success' not in msg.lower():
                            raise Exception(f"源目录列表失败: {msg}")
                    except Exception as e:
                        if '源目录列表失败' in str(e):
                            raise
                        # 获取完整响应失败，按原逻辑处理
                        pass
                break
            allItems.extend(content)
            total = res.get('total', 0)
            if len(allItems) >= total or len(content) < perPage:
                break
            page += 1
            firstPage = False
        if allItems:
            rts = {
                ...
                else (item['size'], item.get('modified', '')) for item in allItems
            }
        else:
            rts = {}
        if spec and rts:
